...
class UndeployActions extends CNAScript {
  async run () {
    ...
    const manifestFile = this.config.manifest.src
    if (!(await fs.exists(manifestFile)) || !(await fs.statSync(manifestFile)).isFile()) {
      throw new Error(`Cannot undeploy actions, missing ${this._relCwd(manifestFile)} file (autogenerated on deploy).`)
    }

    await utils.spawnAioRuntimeDeploy(this.config.manifest.dist, 'undeploy')

    // rm autogen manifest
    await fs.unlink(this.config.manifest.dist)
