...
const aioLogger = require('@adobe/aio-lib-core-logging')('@adobe/aio-app-scripts:config-loader', { provider: 'debug' })
...
module.exports = () => {
  ...
  if (config.app.hasBackend) {
    ...
    // TODO: migrate existing projects that have the packagePlaceholder in their manifest.yml
    config.manifest.full = yaml.safeLoad(fs.readFileSync(config.manifest.src, 'utf8'))
    if (config.manifest.full.packages[config.manifest.packagePlaceholder]) {
      aioLogger.warn(`Warning: use of ${config.manifest.packagePlaceholder} in manifest.yml is deprecated.`)
      config.manifest.package = config.manifest.full.packages[config.manifest.packagePlaceholder]
      config.manifest.package.name = `${config.app.name}-${config.app.version}`
      aioLogger.info('generating package name = ', config.manifest.package.name)
    } else {
      ...
      config.manifest.package.name = packageNames[0]
    }
    aioLogger.info('config.manifest.package.name = ', config.manifest.package.name)
  }

  // 5. deployment config
  config.ow = Object.assign({
    apihost: defaultOwApiHost,
    apiversion: 'v1',
    package: config.app.hasBackend ? config.manifest.package.name : ''
  }, userConfig.runtime)
  // s3
