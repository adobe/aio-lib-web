...
module.exports = () => {
  ...
  // 2.c. needed files
  if (!fs.existsSync(config.manifest.src) || !fs.lstatSync(config.manifest.src).isFile()) throw new Error('Missing manifest.yml file')
  if (!fs.existsSync(_abs('package.json')) || !fs.lstatSync(_abs('package.json')).isFile()) throw new Error('Missing package.json file')
