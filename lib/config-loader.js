...
module.exports = () => {
  ...
  // 2. checks
  if (config.actions.remote) {
    debug('running config checks')
    if (Object.keys(userConfig).length === 0) {
      throw new Error('Missing aio config (set .aio file or AIO_XXX env vars)')
    }
    // 2.a missing runtime config
    if (typeof userConfig.runtime !== 'object') {
      throw new Error('Missing aio runtime config (set AIO_RUNTIME_XXX env vars)')
    }
    if (!userConfig.runtime.apihost) {
      throw new Error('Missing runtime.apihost .aio config (or AIO_RUNTIME_APIHOST env)')
    }
    if (!userConfig.runtime.auth) {
      throw new Error('Missing runtime.auth .aio config (or AIO_RUNTIME_AUTH env)')
    }
    if (!userConfig.runtime.namespace) {
      throw new Error('Missing runtime.namespace .aio config (or AIO_RUNTIME_NAMESPACE env')
    }
    // 2.b missing tvm/s3 creds config if has frontend
    if (config.app.hasFrontend && !userConfig.cna.tvmurl && !config.s3.creds) {
      throw new Error('Missing cna.tvmUrl .aio config (or AIO_CNA_TVMURL env) or aws credentials')
    }
  } else {
    debug('skipping some config checks for local build')
  }
  // 2.c. needed files
  if (!fs.existsSync(config.manifest.src) || !fs.lstatSync(config.manifest.src).isFile()) {
    throw new Error('Missing manifest.yml file')
  }
  if (!fs.existsSync(_abs('package.json')) || !fs.lstatSync(_abs('package.json')).isFile()) {
