...
const propsReader = require('@adobe/aio-cli-plugin-runtime/src/properties')
...
module.exports = () => {
  ...
  // read from ~/.wskprops or WSK_CONFIG_FILE or OW env vars
  const props = propsReader.propertiesFile()
  if (props &&
      props._properties &&
      props._properties.AUTH &&
      props._properties.NAMESPACE &&
      props._properties.APIHOST) {
    config.ow.auth = props._properties.AUTH
    config.ow.namespace = props._properties.NAMESPACE
    config.ow.apihost = props._properties.APIHOST
  }
