...
async function deployWsk (packageName, manifestPath, manifestContent, ow, logger, filterEntities) {
  function _filterOutEntities (entities, filters, keys) {
    keys.forEach(k => {
      filters[k] = filters[k] || []

      // special case for actions, entities.actions are prefixed with packageName
      const transformedFilter = k === 'actions' ? filters[k].map(name => `${packageName}/${name}`) : filters[k]

      entities[k] = entities[k].filter(e => transformedFilter.includes(e.name))
    })
  }
  ...
  if (typeof filterEntities === 'object') {
    ...
    filterEntities.pkgAndDeps = [packageName, ...(filterEntities.deps || [])]

    _filterOutEntities(entities, filterEntities, ['actions', 'apis', 'triggers', 'rules', 'pkgAndDeps'])
  }
...
async function undeployWsk (packageName, manifestContent, ow, logger) {
  ...
  return ioruntime.undeployPackage(entities, ow, logger)
}
