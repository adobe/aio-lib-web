...
function zip (filePath, out, pathInZip = false) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (stats.isDirectory()) {
      ...
      // e.g. symlinks
      reject(new Error(`${filePath} is not a valid dir or file`))
    }
...
async function runOpenWhiskJar (jarFile, apihost, waitInitTime, waitPeriodTime, timeout, execaOptions = /* istanbul ignore next */ {}) {
...
function getUIDevExpressApp (entryFile, outDir) {
  /* istanbul ignore next */ // ignoring as cannot be reached from scripts, to be tested if standalone
...
function getActionUrls (owConfig, manifestPackageContent, /* istanbul ignore next */ isLocalDev = false) {
  // 6. set action urls
  ...
  return Object.entries({ ...manifestPackageContent.actions, ...(manifestPackageContent.sequences || {}) }).reduce((obj, [name, action]) => {
    ...
    if (isLocalDev) {
      ...
      obj[name] = urlJoin(owConfig.apihost, 'api', owConfig.apiversion, webUri, owConfig.namespace, owConfig.package, name)
    } else {
      // https://<ns>.adobeioruntime.net/api/v1/web/<package>/<action>
      obj[name] = urlJoin('https://' + owConfig.namespace + '.' + removeProtocolFromURL(owConfig.apihost), 'api', owConfig.apiversion, webUri, owConfig.package, name)
    }
    return obj
  }, {})
}
