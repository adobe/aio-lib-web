...
async function runOpenWhiskJar (jarFile, apihost, waitInitTime, waitPeriodTime, timeout, execaOptions = {}) {
...
function generateActionUrls (owConfig, manifestPackageContent, isLocalDev = false) {
  // 6. set action urls
  ...
  return Object.entries({ ...manifestPackageContent.actions, ...(manifestPackageContent.sequences || {}) }).reduce((obj, [name, action]) => {
    ...
    if (isLocalDev) {
      ...
      obj[name] = urlJoin('https://' + owConfig.namespace + '.' + removeProtocolFormURL(owConfig.apihost), 'api', owConfig.apiversion, webUri, owConfig.package, name)
    }
    return obj
  }, {})
}
...
function getEntryFileName (pacakgejson) {
  if (fs.existsSync(pacakgejson)) {
