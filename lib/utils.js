...
async function deployWsk (packageName, manifestPath, manifestContent, ow, logger, filterEntities) {
   aioLogger.debug('Deploying')
  // extract all entities to deploy from manifest
  const packages = manifestContent.packages
  ...
  if (typeof filterEntities === 'object') {
    ...
    filterEntities.actions = filterEntities.actions || []
    filterEntities.triggers = filterEntities.triggers || []
    filterEntities.rules = filterEntities.rules || []
    filterEntities.apis = filterEntities.apis || []
    filterEntities.deps = filterEntities.deps || []

    // always deploy current package
    filterEntities.deps.push(packageName)

    // set action name as it is set by processPackage
    filterEntities.actions = filterEntities.actions.map(name => `${packageName}/${name}`)

    entities.actions = entities.actions.filter(a => filterEntities.actions.includes(a.name))
    entities.triggers = entities.triggers.filter(t => filterEntities.triggers.includes(t.name))
    entities.rules = entities.rules.filter(r => filterEntities.rules.includes(r.name))
    entities.apis = entities.apis.filter(a => filterEntities.apis.includes(a.name))
    entities.pkgAndDeps = entities.pkgAndDeps.filter(p => filterEntities.deps.includes(p.name))
  }

  // do the deployment, manifestPath needed for creating a project hash
  await ioruntime.syncProject(packageName, manifestPath, manifestContent, entities, ow, logger, deleteOldEntities)
  return entities
}
