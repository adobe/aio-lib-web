...
async function deployWsk(packageName, manifestPath, manifestContent, owOptions, logger, filterEntities) {
    ...
    const entities = ioruntime.processPackage(packages, {}, {}, {}, false, owOptions)
...
/**
 * Zip a file/folder using archiver, with zipping options
 * @param {String} filePath
 * @param {String} out
 * @param {boolean} pathInZip
 * @returns {Promise}
 */
function zipWithOptions(filePath, out, zipOptions, pathInZip = false) {
    if(!zipOptions){ // call standard zipping
        return zip(filePath, out, pathInZip);
    } 

    aioLogger.debug(`Creating zip of file/folder ${filePath} with zip options ${zipOptions}`)
    const stream = fs.createWriteStream(out)
    const archive = archiver('zip', { zlib: { level: 9 } })
    ...
    return new Promise((resolve, reject) => {
        ...
        for(const item of zipOptions.additionalFiles){
            console.log('^*^^^^^^^^^^^^^^^^^^^^^^^^^^')
            console.log(item)
            archive.append(item.destination, {name: item.filename})
            console.log('^*^^^^^^^^^^^^^^^^^^^^^^^^^^')
        }
