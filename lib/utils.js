...
async function undeployWsk (packageName, manifestContent, owOptions, logger) {
  ...
  const manifestEntities = ioruntime.processPackage(manifestContent.packages, {}, {}, {}, true)
...
/**
 * Turns glob into an awaitable function
 * @param {String} pattern glob pattern to match
 * @returns {Promise}
 */
async function getMatchingFileList (pattern) {
...
async function getIncludesForAction (action) {
  ...
  if (action.include) {
    ...
    const files = await Promise.all(action.include.map(async elem => {
      ...
      pair.sources = await getMatchingFileList(elem[0])
      return pair
    }))
