...
function zip (filePath, out, pathInZip = false) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const stats = fs.statSync(filePath) // throws if enoent
...
async function downloadOWJar (url, outFile, logger) {
  ...
  logger('successfully downloaded OpenWhisk standalone jar')
}
...
function checkMissingFile (filePath) {
  try {
    if (!fs.lstatSync(filePath).isFile) throw Error(`${path.basename(filePath)} should be a valid file (not a dir or symlink)`)
  } catch (e) {
    if (e.code === 'ENOENT') throw Error(`missing required ${path.basename(filePath)} file`)
  }
}
