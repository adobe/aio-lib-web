...
async function deployWsk (scriptConfig, manifestContent, logger, filterEntities) {
  ...
  if (scriptConfig.app.hasFrontend) {
    ...
    entities.actions.forEach(a => {
      console.log(a.exec.components)
      const needsReplacement = a.exec && a.exec.kind === 'sequence' && a.exec.components.includes(DEFAULT_VALIDATOR)
      if (needsReplacement) {
