...
// todo: read stage/prod from config and generate the url dynamically
// allow .env setting for the url, for localhost, and/or stage->dev.adobeio-static.net 

module.exports = class RemoteStorage {

  /*
curl 'http://localhost:3000/cdn-api/namespaces/development-918-cdntestappgreen-stage/files/' \
  --request DELETE \
 ...
 */
  ...
  async folderExists (prefix, appConfig) {
    ...
    /*
curl 'http://localhost:3000/cdn-api/namespaces/development-918-cdntestappgreen-stage/files/test.txt' \
  --request DELETE \
  -H 'authorization: Bearer ...'
*/
   ...
   */
  async uploadFile (file, filePath, appConfig, distRoot, authToken) {
    ...
    const filePathForServer = filePath === '' ? fileName : `${filePath}/${fileName}`
    const data = {
      file: {
  ...
  async uploadDir (dir, basePath, appConfig, postFileUploadCallback) {
    ...
    while (fileBatch.length > 0) {
      ...
      const res = await Promise.all(fileBatch.map(async file => {
        ...
        const s3Result = await this.uploadFile(file, filePath, appConfig, dir, this.#authToken)
  ...
  /**
   * Joins url path parts using URL() methods
