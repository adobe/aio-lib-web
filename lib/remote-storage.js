...
let deploymentServiceUrl
if (process.env.AIO_DEPLOYMENT_SERVICE_URL) {
  deploymentServiceUrl = process.env.AIO_DEPLOYMENT_SERVICE_URL
} else {
  deploymentServiceUrl = getCliEnv() === PROD_ENV
 ...
 */
  ...
  async folderExists (prefix, appConfig) {
    ...
    const response = await deployServiceFetch(`${deploymentServiceUrl}/cdn-api/namespaces/${appConfig.ow.namespace}/files`, {
      method: 'GET',
      headers: {
