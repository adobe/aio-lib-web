...
module.exports = class RemoteStorage {
  ...
  async uploadFile (file, prefix, appConfig, distRoot) {
    ...
    console.log('cacheControlString is', cacheControlString) // --- IGNORE ---
    const uploadParams = {
      Bucket: this.bucket,
    ...
    console.log('Metadata is', uploadParams.Metadata) // --- IGNORE ---
  ...
  canAddHeader (file, distRoot, rule) {
    ...
    // is there a wildcard matching lib we can use for this? -jm minimatch?
    if (rule === '/*') { // all content
      return true
  ...
  _getCacheControlConfig (mimeType, appConfig) {
    ...
    if (!mimeType) {
      ...
      console.log('html cache duration is', appConfig.htmlCacheDuration) // --- IGNORE ---
    ...
    } else if (false && mimeType.startsWith('image')) {
